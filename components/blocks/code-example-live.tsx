...
export const CodeExampleBlockLive: React.FC<Props> = ({ code, language, stdin }) => {
  ...
  return (
    <div className='border rounded p-4 mb-8'>
      ...
      <button onClick={runCode} disabled={loading} className='mt-3 px-4 py-2 bg-teal-600 text-white rounded hover:bg-teal-700 disabled:opacity-50'>
        {loading ? 'Running...' : 'Run'}
      </button>

      {output && <div className='mt-4 bg-gray-100 dark:bg-gray-800 p-3 rounded font-mono whitespace-pre-wrap'>{output}</div>}
    </div>
  );
};
