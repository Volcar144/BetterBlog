...
export const SearchButton: React.FC<SearchButtonProps> = ({ className, ...buttonProps }) => {
  ...
  return (
    <Button type='button' variant='outline' className={cn(baseClassName, className)} aria-label='Open search' {...buttonProps}>
      ...
      <div className='hidden md:flex gap-0.5'>
        <kbd
          className={`h-5 min-w-5 rounded grid place-items-center bg-muted text-xs text-muted-foreground transition-all duration-200 ${
            isModifierPressed ? 'inset-shadow-sm inset-shadow-foreground/30' : 'shadow-none'
          }`}
        >
          {modifierLabel}
        </kbd>
        <kbd
          className={`h-5 min-w-5 rounded grid place-items-center bg-muted text-xs text-muted-foreground transition-all duration-200 ${
            isKPressed ? 'inset-shadow-sm inset-shadow-foreground/30' : 'shadow-none'
          }`}
        >
          K
        </kbd>
...
const AlgoliaLogo = ({ size = 150 }: { size?: number | string }) => (
  <svg width='80' height='24' aria-label='Algolia' role='img' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 2196.2 500' style={{ maxWidth: size }}>
    <defs>
...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose, children }) => {
  ...
  return createPortal(
    <div className='fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-start justify-center md:pt-[10vh] dark:bg-black/60' onClick={onClose}>
      <div
...
const HitsList = memo(function HitsList({ hits, selectedIndex, attributes, onHoverIndex, hoverEnabled, sendEvent, openResultsInNewTab = true }: HitsListProps) {
        ...
        return (
          ...
          >
            {hasImage ? (
              <div className='w-[100px] h-[100px] self-start flex-[0_0_100px] items-center justify-center overflow-hidden rounded-sm bg-muted'>
                {!isImageFailed ? (
                  ...
                  <div className='flex items-center justify-center w-full h-full text-muted-foreground' aria-hidden='true'>
                    <SearchIcon />
                  </div>
                )}
              </div>
            ...
            <div>
              ...
              {mapping.secondaryText ? <p className='text-sm mt-2 text-muted-foreground'>{getByPath<string | number>(hit, mapping.secondaryText)}</p> : null}
              {mapping.tertiaryText ? <p className='text-sm text-muted-foreground mt-2'>{getByPath<string | number>(hit, mapping.tertiaryText)}</p> : null}
            </div>
          </a>
        );
      })}
  ...
  return (
    ...
    >
      ...
      <div className='flex items-center gap-2 ml-auto'>
        ...
        <Button type='button' variant='outline' className='px-2 text-muted-foreground' onClick={props.onClose}>
          esc
        </Button>
      </div>
    </search>
...
const NoResults = memo(function NoResults({ query, onClear }: NoResultsProps) {
  return (
    <div className='flex flex-col items-center justify-center gap-2 bg-muted p-4 h-[91vh] md:h-[50vh] text-foreground'>
      ...
      <p className='text-sm text-muted-foreground'>Try a different query or ask AI to help.</p>
...
const ResultsPanel = memo(function ResultsPanel({ query, selectedIndex, config, onHoverIndex, scrollOnSelectionChange = true, sendEvent }: ResultsPanelProps) {
  const { items } = useHits();
  ...
  return (
    <>
      <div ref={containerRef} className='flex flex-col h-[91vh] md:h-[50vh] bg-muted gap-4 p-2 overflow-y-auto' role='listbox'>
        <HitsList
...
export function SearchModal({ onClose, config }: SearchModalProps) {
  ...
  const { selectedIndex, moveDown, moveUp, activateSelection, hoverIndex, selectionOrigin } = useKeyboardNavigation(
    items,
    query,
...
const Footer = memo(function Footer() {
  const basePoweredByUrl = 'https://www.algolia.com/developers?utm_medium=referral&utm_content=powered_by&utm_campaign=sitesearch';
  const poweredByHref = typeof window !== 'undefined' ? `${basePoweredByUrl}&utm_source=${encodeURIComponent(window.location.hostname)}` : basePoweredByUrl;
  return (
